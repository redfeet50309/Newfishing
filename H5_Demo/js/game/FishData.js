...
const FISH_TYPES = [
    ...
    { id: 'f_1_1', region: 1, rarityWeight: 1, name: '綠藻小蝌蚪 (Common)', description: '極度弱小，拉力平緩。', pullStrength: 5, behaviorInterval: 120, staminaMax: 50, color: '#98FB98', sellPrice: 5 },
    { id: 'f_1_2', region: 1, rarityWeight: 1, name: '平穩鯉魚 (Common)', description: '平庸的淡水魚。', pullStrength: 10, behaviorInterval: 100, staminaMax: 100, color: '#FFB347', sellPrice: 10 },
    { id: 'f_1_3', region: 1, rarityWeight: 2, name: '急躁鯽魚 (Uncommon)', description: '偶爾會短暫抽動。', pullStrength: 15, behaviorInterval: 60, staminaMax: 80, color: '#B0C4DE', sellPrice: 20 },
    ...
    { id: 'f_2_1', region: 2, rarityWeight: 1, name: '銀色吻仔魚 (Common)', description: '群聚性小魚。', pullStrength: 12, behaviorInterval: 100, staminaMax: 90, color: '#F0F8FF', sellPrice: 15 },
    { id: 'f_2_2', region: 2, rarityWeight: 1, name: '竹莢魚 (Common)', description: '常見的國民海鮮。', pullStrength: 15, behaviorInterval: 90, staminaMax: 120, color: '#778899', sellPrice: 20 },
    { id: 'f_2_3', region: 2, rarityWeight: 2, name: '兇猛石斑 (Uncommon)', description: '底棲魚類，拉力沉重。', pullStrength: 22, behaviorInterval: 120, staminaMax: 220, color: '#8B4513', sellPrice: 40 },
    ...
    { id: 'f_3_1', region: 3, rarityWeight: 1, name: '條紋小丑魚 (Common)', description: '靈活的小型魚。', pullStrength: 18, behaviorInterval: 80, staminaMax: 140, color: '#FF8C00', sellPrice: 30 },
    { id: 'f_3_2', region: 3, rarityWeight: 1, name: '刺蝟河豚 (Common)', description: '生氣時增加瞬間水阻。', pullStrength: 25, behaviorInterval: 60, staminaMax: 160, color: '#F4A460', sellPrice: 35 },
    { id: 'f_3_3', region: 3, rarityWeight: 2, name: '華麗獅子魚 (Uncommon)', description: '帶有毒刺，拉力詭譎。', pullStrength: 28, behaviorInterval: 50, staminaMax: 200, color: '#BA55D3', sellPrice: 70 },
    ...
    { id: 'f_4_1', region: 4, rarityWeight: 1, name: '深海盲鰻 (Common)', description: '擅長連續小幅掙扎。', pullStrength: 28, behaviorInterval: 40, staminaMax: 220, color: '#A9A9A9', sellPrice: 50 },
    { id: 'f_4_2', region: 4, rarityWeight: 1, name: '發光燈籠魚 (Common)', description: '深海常見指標生物。', pullStrength: 30, behaviorInterval: 70, staminaMax: 260, color: '#00FA9A', sellPrice: 60 },
    { id: 'f_4_3', region: 4, rarityWeight: 2, name: '遠古腔棘魚 (Uncommon)', description: '拉力沉重遲緩。', pullStrength: 38, behaviorInterval: 110, staminaMax: 400, color: '#2F4F4F', sellPrice: 120 },
    ...
    { id: 'f_5_1', region: 5, rarityWeight: 1, name: '幽靈水母 (Common)', description: '突然消失又出現的拉力。', pullStrength: 40, behaviorInterval: 20, staminaMax: 300, color: '#E6E6FA', sellPrice: 100 },
    { id: 'f_5_2', region: 5, rarityWeight: 1, name: '寶箱怪魚 (Common)', description: '偽裝成寶箱，體力極高。', pullStrength: 35, behaviorInterval: 90, staminaMax: 600, color: '#DAA520', sellPrice: 150 },
    { id: 'f_5_3', region: 5, rarityWeight: 2, name: '裝甲鄧氏魚 (Uncommon)', description: '頭部覆蓋骨甲防禦力驚人。', pullStrength: 55, behaviorInterval: 120, staminaMax: 800, color: '#8B0000', sellPrice: 280 },
...
function getRandomFish(currentRegionId, maxRodRarity, rareSpawnBonus = 0) {
    ...
    // 如果因某種原因（例如出bug）沒魚可釣，退回最初階的蝌蚪
    if (availableFish.length === 0) {
        availableFish.push(FISH_TYPES[0]);
    }

    // 權重隨機算法 (Weighted Random)
    // 預設每隻魚權重為 1 落點 (或 100%)。如果該魚 rarityWeight >= 3 (Rare 以上)，則權重增加 rareSpawnBonus。
    let totalWeight = 0;
    const weightedPool = availableFish.map(fish => {
        let weight = 1.0;
        if (fish.rarityWeight >= 3) {
            weight += rareSpawnBonus; // e.g., 1.0 + 0.18 = 1.18
        }
        totalWeight += weight;
        return { fish, weight };
    });
